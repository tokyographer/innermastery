(function ($) {
    new Vue({
                ...
                if (me.start_date !== "") {
                    ...
                    for (var ix in me.extra_price) {
                        ...
                        if (item.enable == 1) {
                            switch (item.type) {
                                case "one_time":
                                    type_total += parseFloat(item.price) * parseFloat(item.quantity);
                                    break;
                                case "per_hour":
                                        type_total += (parseFloat(item.price) * parseFloat(item.quantity)) * Math.max(duration_in_hour,24);
                                    break;
                                case "per_day":
                                        type_total += (parseFloat(item.price) * parseFloat(item.quantity)) * Math.max(1,duration_in_day) ;
                                    break;
                            }
        ...
        methods:{
            ...
            addExtraPrice(index){
                this.extra_price[index].quantity ++;
                console.log(this.extra_price);
            },
            minusExtraPrice(index){
                if(this.extra_price[index].quantity  >=2){
				    this.extra_price[index].quantity --;
                }
            },
            doSubmit:function (e) {
