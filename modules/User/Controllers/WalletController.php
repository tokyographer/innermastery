...
class WalletController extends FrontendController
{
    ...
    public function buyProcess(Request $request){
        ...
        $rules = [];
        $message = [];
        if(empty($request->input('deposit_amount'))){
            if(empty($request->input('deposit_option_write'))){
        ...
        if(empty($request->input('deposit_amount'))){
            $wallet_deposit_lists = setting_item_array('wallet_deposit_lists',[]);
            if(!empty($request->input('deposit_option_write'))){
                $deposit_option = $request->input('deposit_option_write');
                $deposit_amount = $deposit_option;
                $deposit_credit = $deposit_option;
                $deposit_option = [
                    "name" => "No Bonus",
                    "amount" => $deposit_option,
                    "credit" => $deposit_option
                ];
            }else{
            ...
            if(intval($deposit_amount) >= 1000){
                $deposit_amount = intval($deposit_amount) + (intval($deposit_amount) * 0.1);
            }
            $deposit_credit = round($deposit_amount, 0, PHP_ROUND_HALF_UP); //* setting_item('wallet_deposit_rate',1);
            if($deposit_amount < 0){
                return redirect()->back()->with("error",__("Deposit option credit is not valid"));
            }
        }
